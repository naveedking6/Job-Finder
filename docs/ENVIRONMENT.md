...
| `JWT_SECRET` | api (Round 2+, auth) | Yes once auth is built | Render env vars (Secret) | Generate with e.g. `openssl rand -hex 32`. Rotate if ever exposed. |
| `JWT_EXPIRES_IN` | api (Round 2+) | No | — | Token lifetime, e.g. `7d`. |
...
| `AUTOMATION_ENABLED` | api (Round 2+) | Yes | Render env vars | The global emergency-stop flag from the brief. Defaults to `false` — automation must be explicitly turned on, never on-by-default. |
